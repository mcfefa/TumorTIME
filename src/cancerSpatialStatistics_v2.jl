...
function TumorTIMEPipeline(file, marker, panelName, panelLoc)
	...
	@time ADResults1 = CSV.read(string(statsdir,"All-Clinical-Data-for-Interdistances-at-Panel1-Tumor2022-10-27.csv"),DataFrame,types=coltypesS)
	ADResults = ADResults1[:,1:12]

	println(ADResults[1:5,:])
...
ReadingFiles = readdir("C:\\Users\\camara.casson\\Dropbox (UFL)\\research-share\\Camara\\ccRCC-TIME-analysis\\data\\Panel-1\\tumor")

for i in 1:size(ReadingFiles)[1]-4
	filenametemp = ReadingFiles[i]
	TumorTIMEPipeline(filenametemp, markerPanel, "Panel-1", "tumor")
end
